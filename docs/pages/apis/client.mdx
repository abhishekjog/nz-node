...
```ts
type Config = {
  user?: string, // default process.env.NZ_USER || process.env.USER
  password?: string or function, //default process.env.NZ_PASSWORD
  host?: string, // default process.env.NZ_HOST
  port?: number, // default process.env.NZ_PORT
  database?: string, // default process.env.NZ_DATABASE || user
  connectionString?: string, // e.g. netezza://user:password@host:5480/database
